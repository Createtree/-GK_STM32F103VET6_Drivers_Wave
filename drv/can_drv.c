...
void CanDrv_Hinit(void)
{
	...
	CAN_InitStructure.CAN_BS1 = CAN_BS1_3tq;//3tq;
	CAN_InitStructure.CAN_BS2 = CAN_BS2_5tq;//5tq;
	...
	else
	{
		...
		while(1)
		{
			...
			case CAN_FLAG_MASK_16:
				...
				//FXR1_HIG = CanDrv_Fiter_Create16bit(p[c].sid_mask,	0,0,ide);
				if(++c < len)
				{
					...
					//FXR2_LOW = CanDrv_Fiter_Create16bit(p[c].sid_id,	0,0,ide);
